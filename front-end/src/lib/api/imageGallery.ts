  ...
  static async load_next_images(user_token: string) {
    ...
    for (let i in image_data) {
      ...
      if (!ImageGallery.images.includes(img.img_url)) {
        //console.log(img)
        const image: Image = {creatorId: img.user.id, uuid: img.url, id: img.id, url: img.img_url, index: ImageGallery.counter, width: img.width, height: img.height, media_type: img.type, stats: img.stats}
