...
{#if downloads.length < 1} 
    ...
    <Table.Root>
        <Table.Header>
            <Table.Row>
                <Table.Head>
                    <Checkbox data-version-id="headerCheckbox" onclick={callback} />
                </Table.Head>
        ...
        <Table.Body>
            {#each downloads as download (download.id)}
                <Table.Row style={ !download.base_model.toLowerCase().includes(needle.toLowerCase()) ? "position: absolute; scale: 0" : ""}>
                    <Table.Cell class="font-medium" style="width: min-content;">
                        <Checkbox data-version-id={download.id} onclick={callback} />
                    </Table.Cell>
                    ...
                    <Table.Cell>
                        ...
                        <h2 class="scroll-m-20 text-xs font-normal tracking-tight pt-2 text-muted-foreground">{download.file_name}</h2>
                        <div class="model-details flex align-middle pt-5 gap-4">
